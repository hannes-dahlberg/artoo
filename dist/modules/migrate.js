...
var migrationPath = path.join(configs_1.default.paths.storage, 'migrations');
var Migrate = (function () {
            ...
            storage_1.storage.checkTable('migrations').then(function (exists) {
                if (!exists) {
                    ...
                    storage_1.storage.getTable('migrations').then(function (rows) {
                        fs.readdir(migrationPath, function (error, files) {
                            if (error) {
                                reject(error);
                                return;
                            }
    ...
    Migrate.create = function (name) {
        return new Promise(function (resolve, reject) {
            fs.writeFile(path.join(migrationPath, helpers.dateFormat() + '-' + name + '.ts'), template, function (error) {
                if (error) {
                    reject(error);
                    return;
                }
