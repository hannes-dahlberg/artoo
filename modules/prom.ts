...
export class Prom {
    static sequence(promises: Array<() => Promise<any>> = [], {mode = Mode.recursive, breakOnResolve = false, breakOnReject = false, output = <Output>{ results: [], resolves: 0, rejects: 0 }}: {mode?: Mode, breakOnResolve?: boolean, breakOnReject?: boolean, output?: Output} = {}): Promise<Output> {
            ...
            if(mode == Mode.recursive) {
                ...
                var nextProm = () => {
                    ...
                    if(promises.length) {
                        ...
                        var promise = Prom.sequence(promises, {mode, breakOnResolve, breakOnReject, output}).then((result: any) => resolve(output));
                ...
                var promise = promises[0]().then((result: any) => {
                    ...
                    if(breakOnResolve) { resolve(output); return; }

                    //Execute next promise
                    nextProm();
                }).catch((error: any) => {
                    //Add error to output
                    output.results.push({ error });
                    output.rejects++;
                    //Resolve if breakOnReject is true
                    if(breakOnReject) { resolve(output); return; }
    ...
    static and(promises: Array<any>, mode: Mode = Mode.recursive) {
        return new Promise((resolve, reject) => {
            //Execute all promises but break on first rejection
            Prom.sequence(promises, { mode, breakOnReject: true }).then((result: any) => {
    ...
    static or(promises: Array<any>, mode: Mode = Mode.recursive) {
        return new Promise((resolve, reject) => {
            //Execute all promises
            Prom.sequence(promises, { mode }).then((result: any) => {
